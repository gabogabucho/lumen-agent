...
### Fixed
- **Memory recall for natural language** (#20): `recall()` now filters Spanish/English stopwords and ORs the significant terms instead of requiring every word to match (FTS5 implicit AND). Conversational questions like "Te acordas de lo que hablamos?" now surface relevant memories; pure-filler queries return the most recent memories instead of nothing.
- **Duplicate inbound WhatsApp messages** (#22): the WhatsApp connector no longer saves inbound messages to memory - `Brain.think` owns conversation persistence. Removes the `whatsapp_message`/`conversation:{session}` duplicate pairs and the FTS noise they caused.

### Added
- **Cross-session continuity** (#21): every turn now injects the latest session summary and up to 10 distilled facts into the system prompt ("What I know from previous sessions"), scoped per user in workspace mode. The agent greets already knowing who it is talking to instead of relying on a lucky recall match.
