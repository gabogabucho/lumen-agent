...
@app.get("/api/modules/{name}/pairing-code")
async def api_modules_pairing_code(request: Request, name: str):
    """Return the WhatsApp pairing code if one has been generated."""
    guard = _require_owner_access(request)
    if guard is not None:
        return guard
    if not _brain:
    ...
    # Only WhatsApp module supports pairing codes
    if name != "x-lumen-comunicacion-whatsapp":
        return JSONResponse(
            status_code=400,
            content={"error": "Pairing code not supported for this module"},
        )

    # Try to read the pairing code generated by the bridge
